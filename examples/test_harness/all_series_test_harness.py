"""
Test Harness for All Series Types

This comprehensive test harness creates multiple charts to test all available
series types in the streamlit-lightweight-charts-pro library. Use this to
quickly verify that all rendering is working correctly.
"""
...
def generate_sample_data():
    ...
    line_data = [
        SingleValueData(times[i], round(base_price + i * 0.5 + random.uniform(-5, 5), 2))
        for i in range(data_points)
    ]
    ...
    ribbon_data = []
    for i in range(data_points):
        base_value = 100 + i * 0.2
        ribbon_data.append(
            {
                "time": times[i],
                "upperBand": round(base_value + 15, 2),
                "middleBand": round(base_value, 2),
                "lowerBand": round(base_value - 15, 2),
...
def main():
    st.set_page_config(
        page_title="Series Test Harness",
        page_icon="📊",
        layout="wide",
    )

    st.title("📊 Streamlit Lightweight Charts Pro - Series Test Harness")
    st.markdown("Comprehensive testing of all available series types and features.")
    ...
    with tab1:
        ...
        area_chart = Chart(
            series=AreaSeries(data=data["line_data"]),
        )
        ...
        bar_chart = Chart(
            series=BarSeries(data=data["ohlcv_data"]),
        )
    ...
    with tab2:
        ...
        histogram_chart = Chart(
            series=HistogramSeries(data=data["volume_data"]),
        )
        histogram_chart.render(key="histogram_test")

        # Baseline
        st.subheader("Baseline Series")
        baseline_chart = Chart(
            series=BaselineSeries(data=data["line_data"]),
        )
        ...
        trend_fill_chart = Chart(
            series=TrendFillSeries(data=data["trend_data"]),
        )
        ...
        band_chart = Chart(
            series=BandSeries(data=data["band_data"]),
        )
    ...
    st.sidebar.markdown("✅ **Advanced Series**: Histogram, Baseline, Trend Fill, Band")
    st.sidebar.markdown("✅ **Multi-Pane**: 2-pane and 3-pane layouts")
    st.sidebar.markdown("✅ **Annotations**: Text, Arrow, Shape annotations")
    st.sidebar.markdown("✅ **Trade Visualization**: Rectangles and markers")
    st.sidebar.markdown("✅ **Signals**: Buy/sell markers")
    st.sidebar.markdown("✅ **Configuration**: Auto-sizing, styling, range switcher")
