...
def create_supertrend_data():
    ...
    for _i in range(90):
        change = rng.normal(0.5, 0.8)  # Positive bias
        base_price += change
        prices.append(base_price)

    # Phase 2: Major downtrend (Apr-Sep 2023)
    for _i in range(180):
        change = rng.normal(-0.8, 0.6)  # Negative bias
        base_price += change
        prices.append(base_price)

    # Phase 3: Recovery with mixed trends (Oct-Dec 2024)
    for _i in range(95):
        change = rng.normal(0.3, 0.4) if _i % 20 < 10 else rng.normal(-0.2, 0.4)
        base_price += change
        prices.append(base_price)
    ...
    for _i, (date, price) in enumerate(zip(dates, prices)):
        ...
        open_price = max(0.01, price + rng.normal(0, volatility * 0.3))
        close_price = max(0.01, price + rng.normal(0, volatility * 0.3))

        # Ensure high is highest and low is lowest
        high_price = max(open_price, close_price, price) + rng.uniform(0, volatility)
        low_price = max(
            0.01,
            min(open_price, close_price, price) - rng.uniform(0, volatility),
        )
        volume = rng.integers(50000, 200000)
...
def create_mock_trend_fill_data(market_data):
    ...
    for i in range(len(market_data)):
        if i < 20:  # First 20 periods
            trend_direction = 1
            trend_line = market_data.iloc[i]["close"] + 5  # Trend line above price for uptrend
        else:
            # Calculate trend based on recent price movement
            recent_prices = market_data.iloc[max(0, i - 20) : i + 1]["close"]
            price_change = recent_prices.iloc[-1] - recent_prices.iloc[0]

            if price_change > 2:  # Uptrend
                trend_direction = 1
                trend_line = market_data.iloc[i]["close"] + 5 + (i % 10) * 0.1  # Above price
            elif price_change < -2:  # Downtrend
                trend_direction = -1
                trend_line = market_data.iloc[i]["close"] - 5 - (i % 10) * 0.1  # Below price
            else:  # Neutral
                trend_direction = 0
                trend_line = None
        ...
        ohlc4 = (
            market_data.iloc[i]["open"]
            + market_data.iloc[i]["high"]
            + market_data.iloc[i]["low"]
            + market_data.iloc[i]["close"]
        ) / 4

        trend_fill_data.append(
            TrendFillData(
                time=int(market_data.iloc[i]["datetime"].timestamp()),
...
def main():
    ...
    # Create data
    market_data = create_supertrend_data()
    trend_fill_data = create_mock_trend_fill_data(market_data)
    ...
    st.write(
        f"**Data Points:** {len(candlestick_data)} candlesticks, "
        f"{len(trend_fill_data)} trend fill points",
    )
