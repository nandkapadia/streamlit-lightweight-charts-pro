...
from streamlit_lightweight_charts_pro.charts.series import LineSeries, AreaSeries
from streamlit_lightweight_charts_pro.data import LineData, AreaData
...
def main():
    ...
    chart_options = ChartOptions(
        ...
        grid=dict(
            vert_lines=dict(color="#e6e6e6"),
            horz_lines=dict(color="#e6e6e6"),
        ),
        crosshair=dict(mode=1),
    )
    ...
    with col1:
        if show_price_pane:
            ...
            st.write(f"• Series: Price Line")
            st.write(f"• Color: Blue (#2196F3)")
    ...
    with col2:
        if show_volume_pane:
            ...
            st.write(f"• Series: Volume Area")
            st.write(f"• Color: Blue with transparency")
    ...
    with col3:
        if show_ma_pane:
            ...
            st.write(f"• Series: MA Line (Dashed)")
            st.write(f"• Color: Orange (#FF9800)")
