...
def run_code_formatting() -> bool:
    ...
    if not run_command(
        ["python", "-m", "autoflake", "tests/", "--remove-all-unused-imports", "--remove-unused-variables", "--recursive", "--in-place"],
...
def run_critical_tests() -> bool:
    ...
    cmd = ["python", "-m", "pytest"] + existing_tests + ["--tb=short", "-q", "--ignore=tests/performance/"]
