...
class TestMemoryProfiling:
    ...
    def test_memory_cleanup_after_series_removal(self, large_dataset):
        ...
        assert memory_change < 50, f"Memory increased by {memory_change:.2f}MB after cleanup, expected < 50MB"
...
class TestConcurrentPerformance:
    ...
    def test_concurrent_series_creation(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            series_list = [future.result() for future in concurrent.futures.as_completed(futures)]
        ...
        start_time = time.perf_counter()
        series_list_seq = [create_series(i) for i in range(n_series)]
        end_time = time.perf_counter()
...
class TestPerformanceRegression:
    ...
    def test_performance_baseline(self, large_dataset):
        ...
        # Test serialization
        start_time = time.perf_counter()
        chart_dict = chart.to_frontend_config()
        end_time = time.perf_counter()
...
def assert_performance_target(operation_name: str, execution_time: float, target_ms: float):
    ...
    assert execution_time <= target_ms, (
        f"[{operation_name}] Execution time {execution_time:.2f}ms "
...
def generate_performance_report(test_results: List[Tuple[str, float, float]]):
    ...
    passed = sum(1 for _, execution_time, target_time in test_results if execution_time <= target_time)
