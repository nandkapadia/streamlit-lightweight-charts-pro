...
def _initialize_component() -> None:
    ...
    if _RELEASE:
        ...
        if frontend_dir.exists():
            ...
            logger.info(
                "Frontend directory exists, attempting to initialize component"
            )
        ...
        logger.info(
            "Development mode: attempting to initialize component with "
            "local server"
        )
        ...
        try:
            # Log successful import of Streamlit components API
            logger.info(
                "Successfully imported streamlit.components.v1 for development"
            )
            ...
            # Streamlit components module couldn't be imported
            logger.exception(
                "Failed to import streamlit.components.v1 for development"
            )
            # Set to None to indicate initialization failed
            _component_func = None
