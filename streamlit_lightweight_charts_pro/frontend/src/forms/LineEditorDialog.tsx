/**
 * @fileoverview TradingView-style Line Editor Dialog
 *
 * This dialog provides comprehensive line editing capabilities with:
 ...
 * - Line style options (solid, dashed, dotted) with live previews
 * - Matches TradingView styling exactly
 */
...
export interface LineConfig {
  ...
  width: number;
  opacity?: number; // Added opacity support
}

/**
 * Props for LineEditorDialog
 */
...
/**
 * Comprehensive color palette matching the TradingView style from the image
 */
const COLOR_PALETTE = [
...
/**
 * Line thickness options with visual representations
 */
...
/**
 * Line styles with visual representations
 */
const STYLE_OPTIONS = [
...
/**
 * Line Editor Dialog component
 */
export const LineEditorDialog: React.FC<LineEditorDialogProps> = ({
  isOpen,
  config,
  onSave,
  onCancel,
  ...
  const [editConfig, setEditConfig] = useState<LineConfig>({
    ...config,
  ...
  const handleThicknessChange = useCallback((width: number) => {
    setEditConfig(prev => ({ ...prev, width }));
  }, []);

  // Handle style change
  const handleStyleChange = useCallback((style: 'solid' | 'dashed' | 'dotted') => {
    setEditConfig(prev => ({ ...prev, style }));
  }, []);
  ...
  const handleCustomColorAreaClick = useCallback((e: React.MouseEvent) => {
  ...
  const dialogContent = (
    <div className="line-editor-overlay" onClick={onCancel} style={{
      position: 'fixed',
      top: 0,
      left: 0,
      right: 0,
              ...
              row.map((color, colIndex) => (
                <button
                  ...
                  style={{
                    ...
                    border: editConfig.color.toUpperCase() === color.toUpperCase()
                      ? '2px solid #2962ff'
            ...
            <div style={{
              ...
              <div
                style={{
                  ...
                  right: 0,
                  bottom: 0,
                  background: `linear-gradient(90deg, transparent 0%, ${editConfig.color} 100%)`
                }}
              />
              <input
                type="range"
            ...
            {THICKNESS_OPTIONS.map((option) => (
              <button
                ...
                style={{
                  flex: 1,
                  height: '28px',
                  border: editConfig.width === option.value
                    ? '2px solid #2962ff'
                    : '1px solid #e0e3e7',
                  borderRadius: '4px',
                  backgroundColor: editConfig.width === option.value
                    ? '#f0f3ff'
                    : '#ffffff',
                  cursor: 'pointer',
                  display: 'flex',
              ...
              >
                <div
                  style={{
                    width: '20px',
                    height: `${Math.max(1, option.value)}px`,
                    backgroundColor: getColorWithOpacity(editConfig.color, editConfig.opacity || 100),
                    borderRadius: option.value > 2 ? '1px' : '0px'
                  }}
                />
              </button>
            ))}
            ...
            {STYLE_OPTIONS.map((option) => (
              <button
                ...
                style={{
                  flex: 1,
                  height: '28px',
                  border: editConfig.style === option.value
                    ? '2px solid #2962ff'
                    : '1px solid #e0e3e7',
                  borderRadius: '4px',
                  backgroundColor: editConfig.style === option.value
                    ? '#f0f3ff'
                    : '#ffffff',
                  cursor: 'pointer',
                  display: 'flex',
              ...
              >
                <div
                  style={{
                    width: '24px',
                    height: '0px',
                    borderTop: `2px ${option.value} ${getColorWithOpacity(editConfig.color, editConfig.opacity || 100)}`
                  }}
                />
              </button>
            ))}
