  ...
  private processQueue(): void {
    ...
    if (nextItem && !this.loading.has(nextItem)) {
      ...
      window.dispatchEvent(
        new CustomEvent('chart-load-ready', {
...
export const ProgressiveChartLoader: React.FC<ProgressiveChartLoaderProps> = React.memo(
  ({ children, chartId, config, onLoadComplete, onLoadError }) => {
    const [loadingState, setLoadingState] = useState<'queued' | 'loading' | 'loaded' | 'error'>(
      'queued'
    );
    ...
    const handleLoadComplete = useCallback(() => {
      ...
      if (process.env.NODE_ENV === 'development') {
        logger.debug(
          `Chart loaded: ${chartId}, Load time: ${loadTime}ms`,
    ...
    useEffect(() => {
      if (loadingStrategy.shouldPreload && loadingState === 'queued') {
        ...
        const preloadTimer = setTimeout(() => {
          ...
          if (process.env.NODE_ENV === 'development') {
            logger.debug(
              `Preloading critical resources for chart: ${chartId}`,
              'ProgressiveChartLoader'
            );
          }
        }, 100);
