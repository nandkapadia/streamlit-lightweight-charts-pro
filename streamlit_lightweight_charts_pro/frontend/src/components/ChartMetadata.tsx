...
export const ChartMetadata: React.FC<ChartMetadataProps> = React.memo(({
  ...
  const computedMetadata = useMemo(() => {
    ...
    const dateRange = metadata.dateRange ? {
      start: typeof metadata.dateRange.start === 'number'
        ? new Date(metadata.dateRange.start).toISOString().split('T')[0]
        : metadata.dateRange.start,
      end: typeof metadata.dateRange.end === 'number'
  ...
  useEffect(() => {
    ...
    try {
      ...
      themeColorMeta.setAttribute('content', computedMetadata.theme === 'dark' ? '#1a1a1a' : '#ffffff');
  ...
  useEffect(() => {
    ...
    if (openGraphTags) {
      openGraphTags.forEach(tag => {
        let metaElement = document.querySelector(`meta[property="${tag.property}"]`) as HTMLMetaElement;
        if (!metaElement) {
...
export function useChartMetadata(chartId: string, initialMetadata: ChartMetadata) {
  ...
  const updateDataInfo = React.useCallback((dataPoints: number, dateRange?: ChartMetadata['dateRange']) => {
    updateMetadata({
      dataPoints,
      ...(dateRange && { dateRange })
    });
  ...
  const updateTheme = React.useCallback((theme: 'light' | 'dark') => {
...
export const ChartMetadataPresets = {
  candlestick: (chartId: string): ChartMetadata => ({
    ...
    description: 'Interactive candlestick chart showing price movements with OHLC data visualization.',
