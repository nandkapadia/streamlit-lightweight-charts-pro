...
export class PrimitiveStylingUtils {
  ...
  static applyTypography(
    ...
    if (config.lineHeight) {
      style.lineHeight = typeof config.lineHeight === 'number' ?
        config.lineHeight.toString() : config.lineHeight
    }
    if (config.letterSpacing) {
      style.letterSpacing = typeof config.letterSpacing === 'number' ?
        `${config.letterSpacing}px` : config.letterSpacing
    }
  }
