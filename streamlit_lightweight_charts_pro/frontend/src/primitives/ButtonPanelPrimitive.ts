...
export class ButtonPanelPrimitive extends BasePanePrimitive<ButtonPanelPrimitiveConfig> {
  ...
  private initializeManagers(): void {
    ...
    if (!this.collapseManager) {
      this.collapseManager = PaneCollapseManager.getInstance(
        this.chart,
        this.config.chartId,
      );
  ...
  private initializeButtons(): void {
    ...
    if (this.config.showCollapseButton !== false) {
      const collapseButton = new CollapseButton({
        ...
        isCollapsed:
          this.collapseManager?.isCollapsed(this.config.paneId) || false,
  ...
  private getButtonStyling(): ButtonStyling {
    return {
      ...
      hoverBackground:
        this.config.buttonHoverBackground || "rgba(255, 255, 255, 1)",
      border: "none",
      borderRadius:
        this.config.buttonBorderRadius ||
        ButtonDimensions.PANE_ACTION_BORDER_RADIUS,
  ...
  private renderButtons(): void {
    ...
    this.reactRoot.render(
      React.createElement(React.Fragment, null, ...buttonElements),
    );
  }
  ...
  protected getPaneId(): number {
    return this.config.paneId !== undefined && this.config.paneId > 0
      ? this.config.paneId
      : 0;
  }
  ...
  public getDimensions(): { width: number; height: number } {
    ...
    const width =
      visibleButtonCount * buttonWidth + (visibleButtonCount - 1) * gap;
  ...
  public getSeriesConfig(seriesId: string): SeriesConfiguration | null {
    return (
      this.dialogManager?.getSeriesConfig(this.config.paneId, seriesId) || null
    );
  }
