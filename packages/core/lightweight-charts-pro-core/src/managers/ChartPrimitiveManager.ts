...
export class ChartPrimitiveManager {
    ...
    try {
      ...
      if (seriesReference) {
        try {
          ...
          logger.warn('Failed to attach legend to series, falling back to pane attachment', 'ChartPrimitiveManager', error);
      ...
      logger.error('Failed to create legend primitive', 'ChartPrimitiveManager', error);
  ...
  public addButtonPanel(
    ...
    try {
      ...
      logger.error('Failed to create button panel with full config, using minimal config', 'ChartPrimitiveManager', error);
      return {
        destroy: () => {},
  ...
  private destroyPrimitive(primitiveId: string): void {
    ...
    if (primitive) {
      try {
        ...
        logger.error('Failed to destroy primitive', 'ChartPrimitiveManager', error);
      }
    }
  }
  ...
  public destroy(): void {
    ...
    for (const [, primitive] of this.primitives) {
      try {
        ...
        logger.error('Failed to detach primitive from pane', 'ChartPrimitiveManager', error);
      }
    }
