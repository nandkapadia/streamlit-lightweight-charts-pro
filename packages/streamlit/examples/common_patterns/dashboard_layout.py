...
def generate_dashboard_data():
    ...
    for _i, (date, price) in enumerate(zip(dates, prices)):
        open_price = price + rng.normal(0, 1)
        close_price = price + rng.normal(0, 1)
        high_price = max(open_price, close_price) + rng.uniform(0, 2)
        low_price = min(open_price, close_price) - rng.uniform(0, 2)
...
st.markdown(
    """
 - **[Chart Positioning Example](../chart_management/chart_positioning_example.py)** -
...
st.markdown("---")
st.markdown(
    "💡 **Tip**: Use `st.columns()` to create professional dashboard layouts with proper spacing",
)
