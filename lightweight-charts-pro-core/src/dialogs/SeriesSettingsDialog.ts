...
export class SeriesSettingsDialog extends BaseDialog {
  ...
  public updateSeries(
    ...
    // Update tabs
    if (this._tabManager) {
      const tabs = this._seriesList.map((s) => ({
        id: s.id,
  ...
  private _createTabs(): HTMLDivElement {
    const tabs: Tab[] = this._seriesList.map((series) => ({
      id: series.id,
      title: series.title || series.type,
    }));
  ...
  private _updateContentContainer(): void {
    ...
    const commonSection = this._createCommonSettings(activeId, config);
   ...
   */
  private _createCommonSettings(seriesId: string, config: SeriesFormConfig): HTMLDivElement {
